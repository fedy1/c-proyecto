...
class PostulateController extends Controller
{
    ...
    public function apply(Vacancy $vacancy)
    {
        if (Auth::check() && Auth::user()->candidate) {
            ...
            if (Postulate::where('candidate_id', $candidateId)->where('vacancy_id', $vacancy->id)->exists()) {
                return redirect()->route('vacancy.index')->with('warning', 'Ya has aplicado a esta vacante anteriormente.');
            }
